...
def plot_noe(filename, line_color=None, line_width='1.0', advanced_coloring=0, single=0, quiet=1, aria=0, per_atom=0,
             per_residue=1,seq_dist=1,bb_only=0):
    """
DESCRIPTION
...
ARGUMENTS
    ...
    seq_dist = integer: Required distance between residues in sequence space. e.g. = 1 removes intraresidue NOEs {default: 1}

    bb_only = integer: Only include backbone-backbone NOEs {default: 0}
...
EXAMPLE
    ...
    single, quiet, aria, per_residue, per_atom, advanced_coloring, seq_dist, bb_only = \
        int(single), int(quiet), int(aria), int(per_residue), int(per_atom), int(advanced_coloring), int(seq_dist), int(bb_only)
            ...
            try:
                ...
                if len(shlex_split) > 6:
                    if shlex_split[1].strip().lower() == shlex_split[9].strip().lower() == "segid":
                        ...
                        distance["dst_resid"] = shlex_split[7].strip()  #shlex_split[10].strip()
                        distance["dst_atom"] = shlex_split[10].strip().replace("^", "'") #shlex_split[13].strip().replace("^", "'")
...
def _draw_restraint(restraints, line_color, line_width, single, quiet, per_residue, per_atom, advanced_coloring,seq_dist,bb_only):
    cnt = 0
    for restraint in restraints["connections"]:
        ...
        resi_dist = abs(int(restraint["src_resid"]) - int(restraint["dst_resid"]))
        if resi_dist < seq_dist:
            return 0
        if bb_only:
            if not set([restraint["src_atom"],restraint["dst_atom"]]).issubset(set(['HN','H','HA','CA','N'])):
                return 0
